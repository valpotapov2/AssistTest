// ════════════════════════════════════════════════════════════
//  ACCOUNT MANAGER  (две роли: TESTER фиксированный, USER активный)
// ════════════════════════════════════════════════════════════
const accountManager = (function() {
  ...
  function applyActiveUser() {
    ...
    if (lbl) lbl.textContent = u.name || u.login || '—';
  }

  function getById(id) { return users.find(a => a.id === id); }
  ...
  function fillUserEditor(u) {
    document.getElementById('accName').value     = u ? u.name     : '';
  ...
  return {
    init() {
      ...
      if (!users.find(a => a.active)) users[0].active = true;
    ...
    toggleDropdown() { open ? this.closeDropdown() : this.openDropdown(); },
    ...
    openDropdown() {
      ...
      renderList();
      fillUserEditor(getById(currentUserId) || users[0]);
      fillTesterEditor();
    },
    ...
    saveUser() {
      const u = getById(currentUserId);
      if (!u) return;
      u.name     = document.getElementById('accName').value.trim();
      u.login    = document.getElementById('accLogin').value.trim();
      u.password = document.getElementById('accPass').value;
      if (document.getElementById('accActive').checked) {
        users.forEach(a => a.active = false);
        u.active = true;
      } else {
        u.active = false;
        if (!users.find(a => a.active)) users[0].active = true;
    ...
    saveTester() {
      tester.name     = document.getElementById('testerName').value.trim();
    ...
    newUser() {
      const u = { id: Date.now().toString(), name: '', login: '', password: '', active: false };
      users.push(u);
      currentUserId = u.id;
      persist();
      renderList();
    ...
    deleteUser() {
      if (users.length <= 1) { toast('Нельзя удалить последнего пользователя', 'warn'); return; }
      const wasActive = !!getById(currentUserId)?.active;
      users = users.filter(a => a.id !== currentUserId);
      if (wasActive) users[0].active = true;
      persist();
